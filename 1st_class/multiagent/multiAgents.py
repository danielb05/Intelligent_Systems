...
class AlphaBetaAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        def max_value(gameState, agent, depth, alpha, beta):
            ...
            for a in gameState.getLegalActions(agent):
                v = max(v, min_value(
                    result(gameState, agent, a), 1, depth, alpha, beta))
                if v > beta:
                    return v
                alpha = max(alpha, v)
            return v
        ...
        def min_value(gameState, agent, depth, alpha, beta):
            ...
            for a in gameState.getLegalActions(agent):
                if (agent == gameState.getNumAgents()-1):
                    v = min(v, max_value(result(gameState, agent, a),
                            0, depth-1, alpha, beta))
                else:
                    v = min(v, min_value(result(gameState, agent, a),
    ...
    # smallest values possible since pacman is MAX
        v = -sys.maxint
        actions = []
        alpha = -sys.maxint
        beta = sys.maxint
